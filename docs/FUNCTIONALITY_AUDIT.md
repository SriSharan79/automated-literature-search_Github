...
| **2. Analyze Literature** | Analyze a PDF or a folder: Docling sectioning, abstract + intro analysis, reference extraction, table/image extraction; choose LLM provider + **model**; **skip fuzzy-title duplicates** before batch processing (logs skipped filenames); records **page count** in the registry (no page limit); auto-enriches metadata from **all download logs** | `Pdf_File_processor.process_pdf_mode_file`, `batch_dedup.batch_process_folder`, `download_log_enrich.enrich_from_download_logs` |
