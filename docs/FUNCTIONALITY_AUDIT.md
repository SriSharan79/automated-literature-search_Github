...
| **5. Evaluate & Enrich** *(new)* | Re-run cheaper passes on an analyzed storage (abstract-only, references-only); build analysis-evaluation DBs; classify a publication title on demand; compute text-comparison metrics (Jaccard / ROUGE / BLEU / Levenshtein / WER) | `Folder_Data_Analyzer.process_abstract/process_references`, `data_evaluator.generate_databases`, `publication_classification.title_classifier.classify_title`, `Lexical_Overlap_Metrics`, `Distance_w_Structural _Alignment` |
...
| **Master Excel DB builder** | Consolidate per-section DBs into one master workbook | `rag_builders/master_excel_db_builder.py` (`save_to_db`) | ⏳ `save_to_db` is a low-level per-entry writer with no top-level driver; needs an orchestrator before a button is meaningful |
| **Question-scored classification sheets** | Score a publication against the full question set into sheets | `publication_classification/Classification_logic_with_Q's` (`classify_excel_data_to_sheets`) | ⏳ needs an Excel-in / sheet-out form |
| **Query-result harvesting** | Copy matching PDFs/JSONs for a query; enrich overview with abstracts; batch enrich reports | `query_executor.harvest_query_resources`, `enrich_overview_with_abstracts`, `batch_enrich_reports` | ⏳ needs a post-query "collect results" step (requires an overview path + VDB/MF context) |
