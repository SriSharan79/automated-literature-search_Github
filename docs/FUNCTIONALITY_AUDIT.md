...
| **5. Evaluate & Enrich** *(new)* | Re-run cheaper passes on an analyzed storage (abstract-only, references-only); build analysis-evaluation DBs; build the consolidated master Excel workbook; classify a publication title on demand; compute text-comparison metrics (Jaccard / ROUGE / BLEU / Levenshtein / WER) | `Folder_Data_Analyzer.process_abstract/process_references`, `data_evaluator.generate_databases`, `master_excel_db_builder.build_master_excel_db`, `publication_classification.title_classifier.classify_title`, `Lexical_Overlap_Metrics`, `Distance_w_Structural _Alignment` |
