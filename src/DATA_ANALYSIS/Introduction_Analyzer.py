...
from DATA_ANALYSIS.Data_analysis_system_prompts import  Intro_RP_KEYs_SP, Introduction_identification_SP
from COMMON.General_Utils import caluculate_time_taken, clean_response_json_text,find_first_match_in_first_n_chars
from COMMON.JSON_file_Utils import get_value_by_pair, pretty_print_json_from_file, store_to_json, get_key_from_file, store_to_json_with_text,print_json_file
...
def get_Introduction_text(MF): 
    Introduction_text=''   
    
    Introduction_text = get_value_by_pair(MF.raw_sec_json_path, 'Section Name','Introduction','Text_Content')
    ...
    if not Introduction_text:
        ...
        # print (Chunks)
        Introduction_text=find_first_match_in_first_n_chars(Chunks[:8],Introduction_list,10)
        if not Introduction_text:
...
def analyze_Introduction(ID,MF):
    ...
    if Introduction_text:
        ...
        llm_service=MF.llm_service
        system_output=clean_response_json_text(llm_call(User_prompt, Intro_RP_KEYs_SP,llm_service))
        end_time=time.time()
        print(Fore.GREEN+"\n Analyzed Introduction Info: \n"+ Style.RESET_ALL)
